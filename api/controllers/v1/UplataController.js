...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      // values.iznos = iznos;
      if (!values.predmet) throw 'Requires predmet'; // eslint-disable-line
      newUplata = await Uplata.create(values);
      ...
      await Predmet.update({ id: values.predmet }, { dug: predmet.dug - values.iznos });

      res.created({ uplata: newUplata });
    } catch (err) {
      if (newUplata) await Uplata.destroy({ id: newUplata.id });
      res.badRequest(err);
    }
  },

  createForUsluga: async (req, res) => {
    try {
      const values = omit(req.allParams(), ['id']);
      values.poslovnica = req.user.poslovnica.id;
      values.user = req.user.id;
      const usluga = await Usluga.findOne({ id: values.usluga });
      if (isEmpty(usluga)) throw 'Usluga does not exist'; // eslint-disable-line
